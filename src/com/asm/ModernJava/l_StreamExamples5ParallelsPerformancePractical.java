...
public class l_StreamExamples5ParallelsPerformancePractical {
    ...
    static {
        ...
        // array는 다찰때마다 1.5배씩증가 -> 메모리가 갑자기 커짐
        final Product1[] list = new Product1[length];

        for (int i = 1; i <= length; i++) {
            list[i - 1] = new Product1((long) i, "Product" + i, new BigDecimal(priceStrings[random.nextInt(5)]));
        }
        products = Collections.unmodifiableList(Arrays.asList(list));
        //unmodifiableList -> 사이즈값 이 고정 (안에 데이터 수정 가능)
    }
